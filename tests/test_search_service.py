...
@pytest.fixture
def mock_es():
    with patch('src.api.search_service.Elasticsearch') as MockEs:
        instance = MagicMock()
...
@pytest.fixture
def mock_model():
    with patch('src.api.search_service.SentenceTransformer') as MockModel:
        instance = MagicMock()
...
def search_service(mock_es, mock_model):
    from src.api.search_service import EcomSearchService
...
class TestEcomSearchService:
    ...
    def test_semantic_search_returns_empty_on_es_error(self, search_service, mock_es, mock_model):
        mock_es.search.side_effect = Exception("ES connection refused")
        results = search_service.semantic_search("test query")
        assert results == []

    def test_semantic_search_result_has_required_fields(self, search_service, mock_es, mock_model):
        mock_es.search.return_value = {
            'hits': {
                'hits': [
                    {
